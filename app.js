...
class App {
    ...
    static async loadRemediesDB() {
        try {
            const res = await fetch(`${CONFIG.HEXAGRAM_FUNCTION_URL}/remedies-db`);
    ...
    static async setLanguage(lang) {
        ...
        if (this.currentReading) {
            ...
            if (needsTranslation) {
                // First render English interpretation so sections exist for translation service to find
                if (this.currentReading.interpretation && typeof UI !== 'undefined') {
                    UI.renderInterpretationTabbed(this.currentReading.interpretation, 'en', this.currentReading);
                    UI.initReadingTabs();
                }
                ...
                if (this.currentReading.interpretation?.remedies && lang !== 'en') {
    ...
    static async fetchTranslationForLanguageChange(targetLang) {
        ...
        try {
            ...
            if (this.currentReading.interpretation.baguaMedicine) {
                ...
                if (!hasBaguaTranslations && targetLang !== 'en') {
    ...
    static async performCast() {
        try {
            ...
            // PARALLEL API FETCH STRATEGY
            // Start Chinese Astrology and AI interpretation in parallel
            console.log('[App.performCast] Starting parallel API calls...');
            this.loadingOverlay.logDebug('Initiating parallel API requests...', 'info');
            
            this.loadingOverlay.addProcess('ai', 'Requesting AI interpretation');
            this.loadingOverlay.addProcess('astrology', 'Querying Chinese astrology data');
            
            const astroPromise = this.fetchChineseAstrologyAsync(this.currentReading);
            const aiPromise = this.fetchAIInterpretation();
            
            // Wait for AI interpretation (primary user-facing content)
            try {
                await aiPromise;
            ...
            // After AI is done, wait for astrology with timeout
            // This ensures we have astrology data for the final saved reading
            try {
                await Promise.race([
                    astroPromise,
                ...
                this.loadingOverlay.logDebug('Astrology data received successfully', 'completed');
                // Re-render analysis tab with astrology data
                this.renderAnalysisTab();
            } catch (e) {
                console.warn('[App.performCast] Astrology fetch incomplete:', e.message);
    ...
    static async fetchAIInterpretation() {
        ...
        // STRATEGY 1: Frontend-driven sequential section fetching with per-section retry.
        // Each section is its own HTTP call to Supabase (own 60s budget).
        // Failed sections are retried individually before giving up.
        let result = null;
        let error = null;
        const t = I18N[this.lang] || I18N['en'];

        console.log('[AI] Starting sequential section fetching (Strategy 1)...');
        UI.renderAILoading(this.lang, t.gatheringWisdom || 'Gathering wisdom...');

        try {
            result = await this.fetchSectionsSequential(baseRequest);
            if (result) console.log('[AI] Sequential sections succeeded');
        } catch (e) {
            console.warn('[AI] Sequential sections failed:', e.message);
            error = e;
        }

        // STRATEGY 2: Fallback to simple endpoint (single server-side call)
        if (!result) {
            try {
        ...
        // STRATEGY 3: Final fallback to legacy
        if (!result) {
            try {
        ...
        if (result) {
            ...
            if (needsTranslation && result.en?.analysis) {
                ...
                try {
                    // Use new TranslationService for section-based translation
                    const readingId = this.currentReading.requestTimestamp;
                    const hexagramName = `${this.currentReading.hex.name_en} (${this.currentReading.hex.name_zh})`;
                    ...
                    try {
                        ...
                        console.error('[AI] Fallback translation also failed:', fallbackError);
                    }
                }
            }

            // Post-process to organize and redact the response
            result = this.postProcessInterpretation(result);
            
            this.currentReading.interpretation = result;
            UI.renderAIInterpretation(result, this.lang);
            // NOTE: Loading animation stays visible until secondary endpoints complete
            ...
            // Wait for current language remedy translation before hiding loading
            await this.fetchSecondaryEndpoints(baseRequest, result);
            
            // Now hide loading animation after all critical translations are complete
            UI.hideAILoading();
        } else {
    ...
    // Fetch sections sequentially from individual endpoints.
    // Each call gets its own Supabase 60s budget. Failed sections retry once.
    // After each section, incrementally compose via /interpret-compose so each
    // compose call only handles one section's worth of translation/merging.
    static async fetchSectionsSequential(baseRequest) {
        console.log('[AI:SEQ] Fetching sections sequentially with incremental compose...');
        const timeoutMs = 55000; // just under Supabase 60s limit
        const composeTimeoutMs = 30000; // Reduced to 30s for compose to avoid timeouts
        ...
        const sectionsData = {};
        let accumulated = null; // grows with each compose call
        let accumulatedContext = ''; // accumulates interpretation context from previous sections (for reference)
        let accumulatedTechnicalData = ''; // accumulates TECHNICAL DATA (simplified markdown) from previous sections

        // Helper: extract TECHNICAL DATA (not interpretations) for next sections
        const extractTechnicalData = (section, data) => {
            if (!data || !data.technicalData) return '';
            
            // Parse the technicalData JSON
            let techData;
            try {
                techData = typeof data.technicalData === 'string' ? JSON.parse(data.technicalData) : data.technicalData;
            } catch (e) {
                return '';
            }
            
            // Simplify to markdown format
            let md = '';
            switch (section) {
                case 'celestial-astro':
                    md = `## Celestial Technical Data\n\n`;
                    if (techData.lunarMansion) {
                        md += `- **Lunar Mansion:** ${techData.lunarMansion.name} (${techData.lunarMansion.element})\n`;
                    }
                    if (techData.taiSui) {
                        md += `- **Tai Sui:** ${techData.taiSui.position}\n`;
                    }
                    break;
                case 'celestial-bazi':
                    md = `## BaZi Technical Data\n\n`;
                    if (techData.currentBazi?.dayMaster) {
                        md += `- **Day Master:** ${techData.currentBazi.dayMaster.name} ${techData.currentBazi.dayMaster.element}\n`;
                        md += `- **Strength:** ${techData.currentBazi.strength}\n`;
                    }
                    break;
                case 'elements-analysis':
                case 'elements-synthesis':
                    md = `## Five Elements Technical Data\n\n`;
                    if (techData.elementCounts) {
                        md += `- **Element Balance:**\n`;
                        Object.entries(techData.elementCounts).forEach(([el, val]) => {
                            md += `  - ${el}: ${val}%\n`;
                        });
                    }
                    if (techData.trigramElements) {
                        md += `- **Trigrams:** Upper ${techData.trigramElements.upper}, Lower ${techData.trigramElements.lower}\n`;
                    }
                    break;
                case 'houtou-emperor':
                case 'houtou-master':
                    md = `## Later Heaven Technical Data\n\n`;
                    if (techData.dayMaster) {
                        md += `- **Day Master:** ${techData.dayMaster}\n`;
                    }
                    break;
                case 'core-technical':
                case 'core-narrative':
                case 'core-application':
                    md = `## Core Technical Data\n\n`;
                    if (techData.hexagram) {
                        md += `- **Hexagram:** ${techData.hexagram.number} - ${techData.hexagram.name}\n`;
                    }
                    if (techData.movingLines?.length) {
                        md += `- **Moving Lines:** ${techData.movingLines.join(', ')}\n`;
                    }
                    break;
                case 'lines':
                    md = `## Lines Technical Data\n\n`;
                    if (techData.movingLines?.length) {
                        md += `- **Moving Lines:** ${techData.movingLines.join(', ')}\n`;
                    }
                    break;
                case 'classical':
                    md = `## Classical Texts Technical Data\n\n`;
                    break;
            }
            return md;
        };

        // Helper: extract interpretation text (old method for reference)
        const extractContext = (section, data) => {
            if (!data) return '';
            let context = '';
            switch (section) {
                case 'celestial':
                case 'celestial-astro':
                    context = `CELESTIAL ASTROLOGY:\n`;
                    if (data.celestial) context += `Overview: ${data.celestial.substring(0, 500)}\n`;
                    break;
                case 'celestial-bazi':
                    context = `CELESTIAL BAZI:\n`;
                    if (data.birthBazi?.readingImpact) context += `Birth BaZi Impact: ${data.birthBazi.readingImpact.substring(0, 300)}\n`;
                    if (data.currentBazi?.readingImpact) context += `Current BaZi Impact: ${data.currentBazi.readingImpact.substring(0, 300)}\n`;
                    break;
                case 'elements-analysis':
                    context = `ELEMENTS TECHNICAL ANALYSIS:\n`;
                    if (data.technicalAnalysis) context += `Technical: ${data.technicalAnalysis.substring(0, 500)}\n`;
                    break;
                case 'elements-synthesis':
                    context = `ELEMENTS INTERPRETATION:\n`;
                    if (data.colloquialInterpretation) context += `Interpretation: ${data.colloquialInterpretation.substring(0, 500)}\n`;
                    break;
                case 'houtou-emperor':
                    context = `HOUTOU EMPEROR ANALYSIS:\n`;
                    if (data.emperorAnalysis) context += `Emperor: ${data.emperorAnalysis.substring(0, 400)}\n`;
                    break;
                case 'houtou-master':
                    context = `HOUTOU MASTER ANALYSIS:\n`;
                    if (data.masterAnalysis) context += `Master: ${data.masterAnalysis.substring(0, 400)}\n`;
                    break;
                case 'core-technical':
                    context = `CORE TECHNICAL ANALYSIS:\n`;
                    if (data.technicalAnalysis) context += `Technical: ${data.technicalAnalysis.substring(0, 500)}\n`;
                    break;
                case 'core-narrative':
                    context = `CORE NARRATIVE ANALYSIS:\n`;
                    if (data.analysis) context += `Narrative: ${data.analysis.substring(0, 500)}\n`;
                    if (data.colloquialInterpretation) context += `Interpretation: ${data.colloquialInterpretation.substring(0, 300)}\n`;
                    break;
            }
            return context;
        };

        // Helper: fetch one section with a single retry, passing accumulated technical data
        const fetchWithRetry = async (section) => {
            try {
                // Pass accumulated technical data as context (not interpretation)
                const sectionRequest = {
                    ...baseRequest,
                    cumulativeTechnicalData: accumulatedTechnicalData
                };
                const data = await this.fetchSection(section, sectionRequest, timeoutMs, null, 3, accumulatedContext);
                if (data) {
                    // Extract technical data (for passing to next sections)
                    const newTechData = extractTechnicalData(section, data);
                    if (newTechData) {
                        accumulatedTechnicalData += '\n' + newTechData;
                    }
                    // Also keep interpretation context for reference
                    const newContext = extractContext(section, data);
                    if (newContext) {
                        accumulatedContext += '\n' + newContext;
                    }
                    return data;
                ...
                console.warn(`[AI:SEQ] ${section} failed: ${e.message}`);
            }
            // Retry once
            console.log(`[AI:SEQ] Retrying ${section}...`);
            try {
                const sectionRequest = {
                    ...baseRequest,
                    cumulativeTechnicalData: accumulatedTechnicalData
                ...
                console.warn(`[AI:SEQ] Retry ${section} failed: ${e2.message}`);
            }
            return null;
        };

        // Helper: incremental compose for one section
        // OPTIMIZED: Don't pass lang - compose returns English only to avoid DeepSeek timeouts
        const incrementalCompose = async (composeSection, sectionData) => {
            const controller = new AbortController();
            const tid = setTimeout(() => controller.abort(), composeTimeoutMs);
            try {
                ...
                const composeRequest = {
                    ...
                    // Note: NOT passing lang - translations handled separately
                };
                let requestBody;
                try {
                    requestBody = JSON.stringify(composeRequest);
                    console.log(`[AI:COMPOSE] ${composeSection} request size: ${requestBody.length} bytes`);
                } catch (serializeError) {
                    console.error(`[AI:COMPOSE] ${composeSection} JSON serialization failed:`, serializeError.message);
                    console.error('[AI:COMPOSE] Section data keys:', Object.keys(sectionData || {}));
                    return false;
                }
                ...
                clearTimeout(tid);
                if (!response.ok) {
                    console.warn(`[AI:SEQ] Compose for ${composeSection} HTTP ${response.status}`);
                    return false;
                }
                const result = await response.json();
                if (result.success) {
                    accumulated = result.data?.interpretation || result.interpretation;
                    console.log(`[AI:SEQ] Compose for ${composeSection} succeeded`);
                    return true;
                }
                console.warn(`[AI:SEQ] Compose for ${composeSection} returned success=false`);
                return false;
            } catch (e) {
                clearTimeout(tid);
                console.warn(`[AI:SEQ] Compose for ${composeSection} error: ${e.message}`);
                return false;
            }
        };

        // Fetch and compose each section incrementally
        let elementsAnalysis = null;
        let elementsSynthesis = null;
        let celestialAstro = null;
        let celestialBazi = null;
        let houtouEmperor = null;
        let houtouMaster = null;
        let coreTechnical = null;
        let coreNarrative = null;
        
        // DISABLED: Batch compose - calling interpret-compose with multiple sections causes timeouts
        // Each section is composed individually to stay under 60s limit
        const composeIndividual = async (name, data) => {
            return await incrementalCompose(name, data);
        };

        for (const section of sections) {
            console.log(`[AI:SEQ] Fetching ${section}...`);
            const data = await fetchWithRetry(section);

            if (!data) {
                console.warn(`[AI:SEQ] ${section} failed after retry, using empty`);
                // Use empty section for compose so structure is complete
                const emptyKey = section.startsWith('elements-') ? 'elements' : 
                                section.startsWith('houtou-') ? 'houtou' :
                                section.startsWith('core-') ? 'core-analysis' :
                                section.startsWith('celestial-') ? 'celestial' : section;
                const emptyData = this.getEmptySection(emptyKey);
                
                if (section === 'celestial-astro') {
                    celestialAstro = emptyData;
                } else if (section === 'celestial-bazi') {
                    celestialBazi = emptyData;
                } else if (section === 'elements-analysis') {
                    elementsAnalysis = emptyData;
                } else if (section === 'elements-synthesis') {
                    elementsSynthesis = emptyData;
                } else if (section === 'houtou-emperor') {
                    houtouEmperor = emptyData;
                } else if (section === 'houtou-master') {
                    houtouMaster = emptyData;
                } else if (section === 'core-technical') {
                    coreTechnical = emptyData;
                } else if (section === 'core-narrative') {
                    coreNarrative = emptyData;
                } else {
                    sectionsData[section] = emptyData;
                    await composeIndividual(section, emptyData);
                }
                continue;
            }

            console.log(`[AI:SEQ] ${section} succeeded`);
            sectionsData[section] = data;

            // Celestial sub-sections: merge before composing
            if (section === 'celestial-astro') {
                celestialAstro = data;
                continue; // wait for bazi
            }
            if (section === 'celestial-bazi') {
                celestialBazi = data;
                // Now merge and compose celestial as one
                const mergedCelestial = {
                    ...(celestialAstro || this.getEmptySection('celestial')),
                    ...celestialBazi
                };
                sectionsData.celestial = mergedCelestial;
                await composeIndividual('celestial', mergedCelestial);
                continue;
            }

            // Elements sub-sections: merge before composing
            if (section === 'elements-analysis') {
                elementsAnalysis = data;
                continue; // wait for synthesis
            }
            if (section === 'elements-synthesis') {
                elementsSynthesis = data;
                // Now merge and compose elements as one
                const mergedElements = {
                    ...(elementsAnalysis || this.getEmptySection('elements')),
                    ...elementsSynthesis
                };
                sectionsData.elements = mergedElements;
                await composeIndividual('elements', mergedElements);
                continue;
            }
            
            // Houtou sub-sections: merge before composing
            if (section === 'houtou-emperor') {
                houtouEmperor = data;
                continue; // wait for master
            }
            if (section === 'houtou-master') {
                houtouMaster = data;
                // Now merge and compose houtou as one
                const mergedHoutou = {
                    ...(houtouEmperor || this.getEmptySection('houtou')),
                    ...houtouMaster
                };
                sectionsData.houtou = mergedHoutou;
                await composeIndividual('houtou', mergedHoutou);
                continue;
            }
            
            // Core sub-sections: merge before composing
            if (section === 'core-technical') {
                coreTechnical = data;
                continue; // wait for narrative
            }
            if (section === 'core-narrative') {
                coreNarrative = data;
                // Now merge and compose core as one
                const mergedCore = {
                    ...(coreTechnical || this.getEmptySection('core')),
                    ...coreNarrative
                };
                sectionsData['core-analysis'] = mergedCore;
                await composeIndividual('core-analysis', mergedCore);
                continue;
            }

            // Regular section: compose individually
            await composeIndividual(section, data);
        }

        // Handle edge case: celestial-astro fetched but bazi never arrived
        if (celestialAstro && !sectionsData.celestial) {
            sectionsData.celestial = celestialAstro;
            await incrementalCompose('celestial', celestialAstro);
        }

        // Handle edge case: elements-analysis fetched but synthesis never arrived
        if (elementsAnalysis && !sectionsData.elements) {
            sectionsData.elements = elementsAnalysis;
            await incrementalCompose('elements', elementsAnalysis);
        }
        
        // Handle edge case: houtou-emperor fetched but master never arrived
        if (houtouEmperor && !sectionsData.houtou) {
            sectionsData.houtou = houtouEmperor;
            await incrementalCompose('houtou', houtouEmperor);
        }
        
        // Handle edge case: core-technical fetched but narrative never arrived
        if (coreTechnical && !sectionsData['core-analysis']) {
            sectionsData['core-analysis'] = coreTechnical;
            await incrementalCompose('core-analysis', coreTechnical);
        }

        // Verify we have core analysis
        if (!sectionsData['core-analysis']?.technicalAnalysis && !sectionsData.core?.technicalAnalysis) {
            throw new Error('Core analysis section failed — cannot compose interpretation');
        }

        // If all compose calls failed, fall back to client-side composition
        if (!accumulated) {
            console.warn('[AI:SEQ] All incremental compose calls failed, falling back to client-side');
            // Fill missing sections with empties
            const required = ['celestial', 'elements', 'core', 'lines', 'classical'];
            required.forEach(s => {
                if (!sectionsData[s]) sectionsData[s] = this.getEmptySection(s);
            });
            return this.clientSideCompose(sectionsData);
        }

        console.log('[AI:SEQ] Incremental compose complete');
        return accumulated;
    }
    ...
    static buildSectionRequest(section, baseRequest) {
        ...
        const { question, hexagram, lines, lang, historyAnalysis, askAgainSource } = baseRequest;
        ...
        const base = { question, hexagram, lines, lang, historyAnalysis, askAgainSource };
        
        // Cumulative technical data from previous sections (simplified markdown format)
        const cumulativeTechData = baseRequest.cumulativeTechnicalData || '';
        ...
        switch (section) {
            ...
            case 'houtou-master':
                return { 
                    ...
                    momentBazi: baseRequest.momentBazi,
            ...
            case 'core-application':
            case 'lines':
                // Include all technical data for comprehensive analysis
                return { 
                    ...base, 
            ...
            case 'remedies-select':
                return { 
                    ...
                    birthBazi: baseRequest.birthBazi, 
                    equilibrium: baseRequest.equilibrium, 
                    interpretation: baseRequest.interpretation, 
                    interpretationContext: baseRequest.interpretationContext,
                    astrology: baseRequest.astrology // Full API astrology data for remedy selection
                };
            ...
            case 'translate':
                // Pass through caller-built request
                return baseRequest;
            case 'fulu-drawing':
                // fetchFuluDrawing already builds the exact request it needs
                return baseRequest;
            default:
                return base;
        }
    ...
    static async fetchSection(section, baseRequest, timeoutMs, externalSignal = null, maxRetries = 3, context = null) {
        ...
        while (attempt < maxRetries) {
            ...
            // Send only the fields this section needs (strips systemInstructions, etc.)
            ...
            try {
                ...
                let requestBody;
                try {
                    requestBody = JSON.stringify(sectionRequest);
                    ...
                    console.error('[AI:FETCH] Request object keys:', Object.keys(sectionRequest));
        ...
        try {
            ...
            const selectRequest = {
                ...
                interpretationContext: {
                    celestialColloquial: interpretation.en?.celestialColloquial || interpretation.celestial || '',
                    elementsColloquial: interpretation.en?.elementsColloquial || interpretation.elements || '',
                    coreColloquial: interpretation.en?.coreColloquial || interpretation.analysis || ''
                }
            };
            ...
            if (selectData && !signal.aborted) {
                console.log('[AI:SECONDARY] Remedies selected successfully');
    ...
    static async fetchRemediesTranslation(selectData, signal) {
        ...
        if (targetLangs.length === 0 && this.lang === 'en') return;

        const langNames = { 'es': 'Spanish', 'it': 'Italian', 'zh': 'Chinese', 'en': 'English' };
        
        // Handle different response structures defensively
        let remedies = [];
        if (selectData.en && selectData.en.remedies) {
            remedies = selectData.en.remedies;
        ...
        // Get relevance context defensively
        const relevance = selectData.en?.relevance || selectData.relevance || '';
    ...
    static async translateRemediesForLanguage(targetLang) {
        if (!this.currentReading?.interpretation?.remedies?.en?.remedies) {
            console.warn('[AI:TRANS] No English remedies to translate from');
            return;
        }
        
        const remedies = this.currentReading.interpretation.remedies.en.remedies;
        const langNames = { es: 'Spanish', it: 'Italian', zh: 'Chinese' };
        const langName = langNames[targetLang] || targetLang;
        
        console.log(`[AI:TRANS] On-demand translation of ${remedies.length} remedies to ${targetLang}`);
        
        for (const remedy of remedies) {
            try {
                console.log(`[AI:TRANS] Translating ${remedy.id} to ${targetLang}...`);
                const result = await this.fetchSection('remedies-translate', {
                    remedy,
                ...
                console.log(`[AI:TRANS] Received result for ${remedy.id}:`, result ? {
                    id: result.id,
                ...
                if (result) {
                    // Ensure all fields are present - use fallback if translation missing fields
                    const completeResult = {
                        ...remedy,  // Original remedy as fallback
                        ...result,   // Translated fields
                        id: remedy.id,  // Ensure ID is preserved
                        type: remedy.type,  // Ensure type is preserved
                        relevance: result.relevance || remedy.relevance || '',  // Ensure relevance is never empty
                        description: result.description || remedy.description || '',
                        instructions: result.instructions || remedy.instructions || '',
                        application: result.application || remedy.application || '',
                        name: result.name || remedy.name || ''
                    };
                    ...
                    // Ensure fuluContentList is preserved
                    ...
                    const idx = langRemedies.findIndex(r => r.id === completeResult.id);
                    if (idx !== -1) {
                        langRemedies[idx] = completeResult;
                        console.log(`[AI:TRANS] Updated existing remedy ${completeResult.id} in ${targetLang}`);
                    } else {
                        langRemedies.push(completeResult);
                        console.log(`[AI:TRANS] Added new remedy ${completeResult.id} to ${targetLang}`);
                    }
                    
                    // Re-render remedies with new translation
                    if (this.lang === targetLang && typeof UI !== 'undefined') {
                        console.log(`[AI:TRANS] Re-rendering remedies for ${targetLang}`);
                        UI.renderRemediesTabbed(this.currentReading.interpretation.remedies, targetLang);
                    }
                    this.updateStoredInterpretation();
                }
            } catch (e) {
                console.warn(`[AI:TRANS] Failed to translate remedy to ${targetLang}:`, e.message);
            }
    ...
    static async translateBaguaMedicineForLanguage(targetLang) {
        if (!this.currentReading?.interpretation?.baguaMedicine?.en) {
            console.warn('[AI:BAGUA:TRANS] No English Bagua Medicine to translate from');
            return;
        }
        ...
        console.log(`[AI:BAGUA:TRANS] Translating Bagua Medicine to ${targetLang}`);
        
        try {
            // Extract content to translate from English
            const enContent = this.currentReading.interpretation.baguaMedicine.en;
            const toTranslate = {
            ...
            const result = await this.fetchSection('translate', {
                content: toTranslate,
                targetLang: targetLang,
                targetLangName: langName,
            ...
            if (result?.translated) {
                ...
                // Re-render if it's the current language
                if (this.lang === targetLang && typeof UI !== 'undefined') {
                    console.log(`[AI:BAGUA:TRANS] Re-rendering Bagua Medicine for ${targetLang}`);
                    UI.renderBaguaMedicine(this.currentReading.interpretation.baguaMedicine, targetLang);
                }
    ...
    static async fetchBaguaMedicine(cleanRequest, interpretation, signal) {
        try {
            ...
            if (baguaData && !signal.aborted) {
                ...
                this.currentReading.interpretation.baguaMedicine = baguaData;
                UI.renderBaguaMedicine(baguaData, this.lang);
                this.updateStoredInterpretation();
            }
        } catch (e) {
            if (e.name !== 'AbortError') {
    ...
    static async fetchDedicatedAdvice(baseRequest, signal) {
        try {
            ...
            if (adviceData && adviceData.advice && !signal.aborted) {
                ...
                console.log('[AI:ADVICE] Advice received:', adviceText.substring(0, 80));
    ...
    static async fetchRemediesVerification(remediesData, signal) {
        try {
            ...
            if (verified && !signal.aborted) {
                ...
                this.currentReading.interpretation.remedies = verified;
                UI.renderRemediesTabbed(verified, this.lang);
                this.updateStoredInterpretation();
            }
        } catch (e) {
    ...
    static async translateInterpretation(readingId, baseContent, targetLang, hexagramName) {
        ...
        try {
            ...
            // Handle new standardized API response format (data.data.translated)
            const translated = data.data?.translated || data.translated;
    ...
    static async translateToLanguage(readingId, result, targetLang, hexagramName) {
        ...
        // Need to translate
        const baseContent = {
            celestialTechnical: result.en.celestialTechnical,
            celestialColloquial: result.en.celestialColloquial,
            celestial: result.en.celestial,

            elementsTechnical: result.en.elementsTechnical,
            elementsColloquial: result.en.elementsColloquial,
            elements: result.en.elements,

            coreTechnical: result.en.coreTechnical,
            coreColloquial: result.en.coreColloquial,
    ...
    static postProcessInterpretation(result) {
        if (!result || !result.en) return result;
        ...
        if (this.currentReading?.chineseAstrology) {
            ...
            container.innerHTML = `
                <div class="astrology-placeholder">
                    ...
                    <p>Calculating celestial influences...</p>
            ...
            if (this.currentReading?.chineseAstrology) {
                ...
                astroContent.innerHTML = `
                    <div class="astrology-placeholder-compact">
                        ...
                        <p>Calculating celestial influences...</p>
                        <p class="sub-text">This may take 10-30 seconds...</p>
                    </div>
                `;
            }
        }
                ...
                <div class="moment-bazi-section">
                    ...
                    <div class="bazi-narrative">
                        ...
                        <div class="narrative-content">
                            ...
                            <p class="narrative-paragraph">
                                <strong>Elemental Balance:</strong> The chart shows ${typeof currentBazi.strength === 'string' ? currentBazi.strength.toLowerCase() : currentBazi.strength} strength,
